---
layout: beer
beer_key: night-night-time
title: "Night Night Time"
status: past_beer
style: Imperial Stout
group: Stout
logo: /assets/images/beer/ostentatious_logo_classic.jpg
description: "Not a beer for the summer, but why should that matter? A big stout. The goal of the beer is to create a strong stout that is dark, chocolaty, leathery but dry and smooth. A fairly complex grain bill that I was inspired to create by reading Mastering Homebrew by Randy Mosher."
malts:
    - 15 lbs Maris Otter Pale malt (3.75 °L)
    - 1.5 lb Brown Malt (60 °L)
    - 0.75 lb Black Patent Malt (525 °L)
    - 0.5 lb Coffee Malt (150 °L)
    - 0.5 lb Chocolate Malt (200 °L)
    - 0.5 lbs Flaked Barley
    - 1.5 lb Corn Sugar
hops:
    - 1 oz Centennial Hops (8.7% AA) - 0 minutes
    - 1 oz Centennial Hops (8.7% AA) - 50 minutes
yeast:
    - 1 package WLP005 British Ale Yeast
other_ingredients:
    - 1 tablet Whirlfloc
    - 9.0 gallons Water
brew_date: "2020-06-06"
target_og: "1.092"
actual_og: "1.096"
target_fg: "1.020"
actual_fg: "1.038"
target_abv: "9.7%"
actual_abv: "7.8%"
calculated_ibu: "25"
calculated_srm: "N/A"
cost: "N/A"
brewing_instructions:
    - Heat 6 gallons of strike water to 164°F
    - Add 6 gallons of water to the mash tun
    - Add all grains to mash tun and confirm temperature of 151°F
    - Steep grains for 60 minutes
    - "While steeping, heat 3 gallons of water to 170°F for lautering"
    - Extract first runnings into boil kettle
    - Add additional 3 gallons water at 170°F to the mash tun
    - "After 15 minutes, collect second runnings"
    - "Total wort volume should be ~6.75 gallons (6.6 actual)"
    - Bring water to a boil and wait for the hot break
    - Add 1 oz Centennial hops
    - "At 45 minutes, dissolve 1.5 lb Corn Sugar"
    - "At 45 minutes, add one tablet Whirlfloc"
    - "At 50 minutes, add 1 oz Centennial hops"
    - "At 60 minutes, chill the wort to approx. 68°F"
    - Move to the primary fermenter
    - Pitch yeast from yeast starter
    - Ferment at 68°F
    - At 12 days cold crash at 38°F
    - "At 14 days, keg and carbonate to XXX vols"
    - "At 17 days, bottle"
starter_instructions:
    - Boil 350 g DME in 3 L of water for 10 minutes
    - Let cool to 68°F
    - Shake vigorously to aerate
    - Pitch 1 package WLP005
    - Let sit for 36 hours shaking occasionally
fermentation_profile:
rating: ""
brewers_notes: []
---
