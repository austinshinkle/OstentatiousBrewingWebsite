---
layout: beer
beer_key: wham-bam
title: "Wham! Bam!"
status: past_beer
style: Strong Ale
group: Strong Ale
logo: /assets/images/beer/ostentatious_logo_classic.jpg
description: "Since I have plenty of beer right now, I thought I would make a barleywine since it will take time to mature in the bottles. I have never tried this style but now that I know about yeast starters I think it will be worth a shot. I will try to make a smaller batch to see if I can get the gravity up. This recipe is based on a clone of Sierra Nevada's Bigfoot Barleywine from the AHA."
malts:
    - 12 lbs Two Row Pale Malt
    - 2 lb 20°L Crystal Malt
hops:
    - 2 oz Centennial Hops (10.7% AA) - 0 minutes
    - 1 oz Cascade Hops (8.1% AA) - 0 minutes
    - 1 oz Cascade Hops (8.1% AA) - 60 minutes
    - 1 oz Chinook Hops (13.2% AA) - 60 minutes
    - 1 oz Chinook Hops (13.2% AA) - 90 minutes
yeast:
    - 2 packages WLP001 California Ale Yeast
other_ingredients:
    - 1 tablet Whirlfloc
    - 7.0 gallons Water
brew_date: "2018-08-15"
target_og: "1.130"
actual_og: "1.090"
target_fg: "1.036"
actual_fg: "1.021"
target_abv: "12.1%"
actual_abv: "9.0%"
calculated_ibu: "N/A"
calculated_srm: "N/A"
cost: "N/A"
brewing_instructions:
    - Heat 5 gallons of strike water to 161°F
    - Add 5 gallons of water to the mash tun
    - Add all grains to mash tun and confirm temperature of 151°F
    - Steep grains for 60 minutes
    - Heat 2 gallons of water to 160°F
    - Extract first runnings into boil kettle
    - Lauter with additional 2 gallons at 160°F
    - "Total wort volume should be ~5.5 gallons"
    - Bring water to a boil and wait for the hot break
    - Add 2 oz Centennial hops
    - Add 1 oz Cascade hops
    - "At 60 minutes, add 1 oz Cascade hops"
    - "At 60 minutes, add 1 oz Chinook hops"
    - "At 80 minutes, add 1 tablet Whirlfloc"
    - "At 90 minutes, add 1 oz Chinook hops"
    - "At 90 minutes, chill the wort to approx. 75°F"
    - Move to the primary fermenter
    - Pitch yeast from yeast starter
    - "At 1 week, rack to the secondary fermenter"
    - "At 3 weeks, prime and bottle beer"
starter_instructions:
    - Boil 1 cup DME in 2 L of solution for 10 minutes
    - Let cool to 75°F
    - Shake vigorously to aerate
    - Pitch 2 packages WLP001
    - Let sit for 36 hours
fermentation_profile:
rating: ""
tasting_notes: []
---
